...
class GamePresenterWidgetState extends State<GamePresenterWidget> with WidgetsBindingObserver {
  static const TIME_STOPPED = 0;

  static final salsaKey = encrypt.Key.fromUtf8(Constant.mateKeyStr);
  static final salsaIv = encrypt.IV.fromUtf8(Constant.mateIvStr);

  static const _KEY_STATE = 'state';

  /// Encrypter to protected saved states of the game and
  /// make hacking a lil bit harder.
  final _encrypter = encrypt.Encrypter(encrypt.Salsa20(salsaKey));
  ...
  void _loadState() async {
    ...
    try {
      ...
      final plainText = _encrypter.decrypt(encrypted, iv: salsaIv);
  ...
  void _saveState() async {
    ...
    final encryptedText = _encrypter.encrypt(plainText, iv: salsaIv).base64;
