...
import 'package:qnc_app/nbpuzzle/data/chip.dart';
import 'package:qnc_app/nbpuzzle/game.dart';
import 'package:qnc_app/nbpuzzle/widgets/game/chip.dart';
...
class _BoardWidgetState extends State<BoardWidget> with TickerProviderStateMixin {
  ...
  late bool _isSpeedRunModeEnabled;
  ...
  void _performSetPrevBoard() => _performSetBoard(newBoard: widget.board!, oldBoard: widget.board);
  ...
  void _performSetBoard({final Board? newBoard, final Board? oldBoard}) {
      ...
      setState(() {
        ...
        void _changeTo(int length) {
          for (var i = 0; i < length; i++) {
            ...
            _onChipChangePosition(chip, wasCurrentPoint, chip.currentPoint, enableColorAnimation: false);

            // Change the color of the chip.
            final color = HSLColor.fromAHSL(1, hueStep * chip.number, 0.7, 0.5).toColor();
    ...
    for (var chip in board.chips) {
      ...
      if (extra.currentPoint != chip.currentPoint || extra.touched) {
        ...
        extra.touched = false;
        extra.currentPoint = chip.currentPoint;
        _onChipChangePosition(chip, wasCurrentPoint, chip.currentPoint, enableColorAnimation: !wasTouched);
      }
    }
  }
  ...
  void _startAppearAnimation(Chip chip) {
    final duration = Duration(milliseconds: _applyAnimationMultiplier(_ANIM_DURATION_BLINK_HALF));
    ...
    controller.forward().then<void>((_) => _disposeAnimation(chip, _ANIM_SCALE_TAG));
  }

  void _startColorBackgroundAnimation(Chip chip, {required Color from, required Color to}) {
    final duration = Duration(milliseconds: _applyAnimationMultiplier(_ANIM_DURATION_COLOR_BACKGROUND));
    final curve = Curves.easeIn;
    ...
    controller.forward().then<void>((_) => _disposeAnimation(chip, _ANIM_COLOR_BACKGROUND_TAG));
  }
  ...
  void _startMoveAnimation(Chip chip, Point<int> point) {
    final controller = AnimationController(
      duration: Duration(milliseconds: _applyAnimationMultiplier(_ANIM_DURATION_MOVE)),
      vsync: this,
    );
    ...
    animation.addListener(() {
      ...
      // of the chip.
      final x = (oldX * (1.0 - animation.value) + point.x * animation.value) / board.size;
      final y = (oldY * (1.0 - animation.value) + point.y * animation.value) / board.size;
    ...
    controller.forward().then<void>((_) => _disposeAnimation(chip, _ANIM_MOVE_TAG));
  }

  void _startBlinkAnimation(Chip chip, Point<int> point) {
    final duration = Duration(milliseconds: _applyAnimationMultiplier(_ANIM_DURATION_BLINK_HALF) * 2);
  ...
  void _startColorOverlayAnimation(Chip chip) {
    final controller = AnimationController(
      duration: Duration(milliseconds: _applyAnimationMultiplier(_ANIM_DURATION_COLOR_OVERLAY)),
      vsync: this,
    );
    ...
    controller.forward().then<void>((_) => _disposeAnimation(chip, _ANIM_COLOR_OVERLAY_TAG));
  }
  ...
  Widget _buildChipWidget(Chip chip) {
    ...
    final backgroundColor = extra.backgroundColor.withOpacity(dst < 1 ? 1 - dst : 0);
  ...
  void onPanStart(BuildContext context, DragStartDetails details) {
    ...
    final aPoint = Point<double>(aPointInt.x.toDouble(), aPointInt.y.toDouble());
    final aPointScaled = aPoint * chipWidgetSize;
    final bPointInt = game.findChipPositionAfterTap(board, point: aPointInt);
    final bPoint = Point<double>(bPointInt.x.toDouble(), bPointInt.y.toDouble());
    ...
    final group = (game.findChips(board, point: activeChip.currentPoint).toList()
          ..sort((a, b) {
            final aDst = a.currentPoint.distanceTo(aPointInt);
            final bDst = b.currentPoint.distanceTo(aPointInt);
            return aDst.compareTo(bDst);
          }))
        .map((chip) => chips!.firstWhere((c) => chip.currentPoint == c.currentPoint))
        .toList();
    ...
    _onPanUpdateDelegate = (double dx, double dy) {
      final x = max(min(activeChip.x * boardWidgetSize + dx, toPointScaled.x), fromPointScaled.x) / boardWidgetSize;
      final y = max(min(activeChip.y * boardWidgetSize + dy, toPointScaled.y), fromPointScaled.y) / boardWidgetSize;
    ...
    _onPanEndDelegate = (double vx, double vy) {
      ...
      final x =
          max(min(activeChip.x * boardWidgetSize + offsetX, toPointScaled.x), fromPointScaled.x) / boardWidgetSize;
      final y =
          max(min(activeChip.y * boardWidgetSize + offsetY, toPointScaled.y), fromPointScaled.y) / boardWidgetSize;
