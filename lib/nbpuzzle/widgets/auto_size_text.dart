...
class AutoSizeTextState extends State<AutoSizeText> {
  ...
  void _sanityCheck(TextStyle style, int maxLines) {
    ...
    assert(maxLines > 0, 'MaxLines has to be grater than or equal to 1.');
    ...
    if (widget.presetFontSizes == null) {
      ...
      assert(
          widget.minFontSize / widget.stepGranularity % 1 == 0, 'MinFontSize has to be multiples of stepGranularity.');
      ...
      assert(
          widget.presetFontSizes != null && widget.presetFontSizes!.isNotEmpty, 'PresetFontSizes has to be nonempty.');
    }
  }
  ...
  List _calculateFontSize(BoxConstraints size, TextStyle style, int maxLines) {
    ...
    if (presetFontSizes == null) {
      var defaultFontSize = style.fontSize!.clamp(widget.minFontSize, widget.maxFontSize);
