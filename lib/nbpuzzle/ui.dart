...
class ConfigUiContainer extends StatefulWidget {
  ...
  static _ConfigUiContainerState? of(BuildContext context) {
    final inherited = context.dependOnInheritedWidgetOfExactType<_InheritedStateContainer>();
...
class _ConfigUiContainerState extends State<ConfigUiContainer> {
  ...
  late bool isSpeedRunModeEnabled;
  ...
  void _loadThemePreferences(final SharedPreferences prefs) {
    ...
    setUseDarkTheme(useDarkTheme ?? false);
  }

  void _loadSpeedRunPreferences(final SharedPreferences prefs) {
    final isSpeedRunModeEnabled = prefs.getBool(_KEY_SPEED_RUN_MODE_ENABLED) ?? this.isSpeedRunModeEnabled;
  ...
  void setSpeedRunModeEnabled(final bool isEnabled, {final bool save = false}) async {
    // Save the choice if we
    // want to.
    if (save) {
      try {
